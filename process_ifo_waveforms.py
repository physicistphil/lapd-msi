...
# Density calculation built by Steve Vincena
f_uwave = 288e9
...
def calc_density(r, s):
    ...
    density = np.unwrap(dphi) * calibration  # units = m^-2

    return density


# Get a numpy array of the densities of the interferometer traces
def get_ifo_densities(path_to_h5):
    densities = tables.open_file(path_to_h5).get_node('/n_L_288GHz').data[:]["density"]
    return densities


# Get a list of datetimes for the interferometer traces
def get_ifo_timestamps(path_to_h5):
    ifo_times = tables.open_file(path_to_h5).get_node('/n_L_288GHz').data[:]["TRIGGER_TIME"]
    ifo_times = np.flip(ifo_times, axis=1)[:, 1:].astype(int)
    ifo_timestamps = [datetime.datetime(*ts) for ts in ifo_times]
    return ifo_timestamps


# Processes interferometer signals (.trc files) and dumps them into an h5 file.
# C2 is the density signal and C1 is the reference signal. They need to be in the same directory.
# The file is saved with the time this script ran.
# You need to pass in the path to the directory containing the.trc files using the --path argument.
#   You can also hardcode this path below.
# You also need to pass in if you want to downsample the density files (defaults to true)

# Interferometer traces are NOT guaranteed to be time-ordered -- this script sorts on the filename
#   of the trc files. Best practice is to sort based on the timestamp.
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process interferometer signals from .trc files')
    parser.add_argument('--path', type=str, default='/path/to/directory/with/trace/files/')
    parser.add_argument('--downsample', type=bool, default=True)

    args = parser.parse_args()

    downsample = vars(args)['downsample']
    path = vars(args)['path']
    # path = "/path/to/directory/with/trace/files/"

    shots_saved = 0

    time_str = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S.%f")
    h5_file = tables.open_file(path + time_str + '.h5', 'w',
    ...
    print("Saving to " + path + time_str + ".h5")

    total_shots = 0
    file_list = sorted(glob.glob(path + "C2*.trc"))
    if file_list:
        for f in file_list:  # so that this can keep up
            ...
            print("{}: Max n_L: {:0.4}\t min n_L: {:0.4}\t time: {}".format(total_shots,
                                                                            np.max(density),
                                                                            np.min(density),
                                                                            ref[0]['TRIGGER_TIME']))
    ...
    print("Saved {} shots to ".format(total_shots) + path + time_str + '.h5')
