...
class SendSettings:
    ...
    server_address (tuple): contains ip address (group) and port number in form [IP, port].
    '''
    ...
    def send_settings(self):
        '''Sends settings to server side to pass through SpecInfo class.'''
        self.sock.setsockopt(socket.IPPROTO_IP, socket.IP_MULTICAST_TTL, 2)
        spec_settings = [self.trig_val, self.int_time_micros]
        self.sock.sendto(b"%a" % spec_settings, (self.server_address[0], self.server_address[1]))

    def send_zero_bytes(self):
        '''Sends zero bytes to server.'''
        self.sock.setsockopt(socket.IPPROTO_IP, socket.IP_MULTICAST_TTL, 2)
        b0 = b""
        self.sock.sendto(b0, self.server_address)
...
class ReceiveSpecData:
    ...
    def bind_socket(self, client_port, server_address):
