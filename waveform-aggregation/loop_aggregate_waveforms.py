...
if __name__ == '__main__':
    ...
    while(True):
        ...
        h5_file = tables.open_file('G:saved_MSI\\Datarun_2024-4-14\\loop\\' + time_str + '.h5', 'w',
                                   title="Monochrometer-" + time_str)
        h5_group = h5_file.create_group("/", "Bdot", "Traces of diamagnetic loop signals")
        h5_tableC1 = h5_file.create_table(h5_group, 'Channel1', Trace, 'Time traces from Channel 1')
        h5_tableC2 = h5_file.create_table(h5_group, 'Channel2', Trace, 'Time traces from Channel 2')
        h5_tableC3 = h5_file.create_table(h5_group, 'Channel3', Trace, 'Time traces from Channel 3')
        h5_rowC1 = h5_tableC1.row
        h5_rowC2 = h5_tableC2.row
        h5_rowC3 = h5_tableC3.row

        print("Saving to G:saved_MSI/Datarun_2024-4-14/loop/" + time_str + ".h5")

        total_shots = 0
        while(total_shots < 3600):
            file_list = sorted(glob.glob("G:saved_MSI\\Datarun_2024-4-14\\loop\\traces\\C3*.trc"))
            if file_list:
                plt.clf()
                for f in file_list:  # so that this can keep up
                    try:
                        # Read last channel first so that all waveforms are on disk
                        C3 = ReadBinaryTrace(f)
                        C2 = ReadBinaryTrace(re.sub(r'\\C3', r'\\C2', f))
                        C1 = ReadBinaryTrace(re.sub(r'\\C3', r'\\C1', f))
                        ...
                        h5_rowC2['time_trace'] = C2[3][:-2]
                        h5_rowC3['time_trace'] = C3[3][:-2]
                        # print(density)
                        for k in C1[0].keys():
                            if k == 'TRIGGER_TIME':
                                h5_rowC1[k] = np.array(C1[0][k])
                                h5_rowC2[k] = np.array(C2[0][k])
                        ...
                        # plt.plot(time_axis, C2[3][:-2], color=[0.0, 1.0, 0.0, 0.3])  # more like BGR
                        # plt.plot(time_axis, C3[3][:-2], color=[0.0, 0.0, 1.0, 0.3])  # more like BGR
                        # plt.tight_layout()
                        # fig.canvas.draw()
                ...
                for f in file_list:
                    try:
                        ...
                        os.remove(os.path.normpath(re.sub(r'\\C3', r'\\C2', f)))  # Remove channel 1
                        os.remove(os.path.normpath(re.sub(r'\\C3', r'\\C1', f)))  # Remove channel 1
                    except Exception as e:
                        print(e)
